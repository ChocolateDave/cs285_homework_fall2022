...
def calculate_mean_prediction_error(
    env, action_sequence, models, data_statistics
):
...
def sample_trajectory(
    ...
    while True:
        ...
        # Render image of the simulation environment
        if render:
            if hasattr(env, 'sim'):
        ...
        # Use the most recent obs to decide what to do
        obs.append(ob)
        ac = policy.get_action(ob)
        ac = ac[0]
        acs.append(ac)

        # Take that action and record transition
        try:
            ob, rew, done, _ = env.step(ac)
...
def sample_trajectories(
    env, policy, min_timesteps_per_batch, max_path_length,
    ...
    paths: List[Mapping[str, np.ndarray]] = []
    while timesteps_this_batch < min_timesteps_per_batch:
        path: Mapping[str, np.ndarray] = sample_trajectory(
...
def sample_n_trajectories(
    env, policy, ntraj, max_path_length,
    render=False, render_mode=('rgb_array')
):
    """
...
def convert_listofrollouts(paths):
    """
        ...
        where each array is a concatenation of
        that array from across the rollouts
    """
...
def eval_trajectories(
    ...
    while timesteps_this_batch < min_timesteps_per_batch:
        ...
        timesteps_this_batch += get_pathlength(path)
        print(
            'At timestep:    ', timesteps_this_batch, '/',
            min_timesteps_per_batch, end='\r'
        )
...
def sample_random_trajectories(
    env, min_timesteps_per_batch, max_path_length,
    render=False, render_mode=('rgb_array')
):
    """
        Collect rollouts using policy
    ...
    while timesteps_this_batch < min_timesteps_per_batch:
        # collect rollout
        path = sample_random_trajectory(
            env, max_path_length, render, render_mode
        )
        paths.append(path)
