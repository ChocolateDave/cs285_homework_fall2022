...
class PiecewiseSchedule(object):
    def __init__(
        ...
        endpoints: [(int, int)]
            ...
            an increasing order.
            When t is between two times, e.g. `(time_a, value_a)`
            and `(time_b, value_b)`, such that `time_a <= t < time_b`
            then value outputs `interpolation(value_a, value_b, alpha)`
            where alpha is a fraction of time passed
            between `time_a` and `time_b` for time `t`.
...
def initialize_interdependent_variables(session, vars_list, feed_dict):
    ...
    while len(vars_left) > 0:
        ...
        if len(new_vars_left) >= len(vars_left):
            ...
            # or more likely if there's another variable outside of the list,
            # that still needs to be initialized. This could be detected here,
            # but life's finite.
            raise Exception(
...
class MemoryOptimizedReplayBuffer(object):
    def __init__(self, size, frame_history_len, lander=False):
        ...
        For the tipical use case in Atari Deep RL buffer with 1M frames the
    ...
    def _encode_observation(self, idx):
        ...
        if start_idx < 0 or missing_context > 0:
            ...
            return self.obs[start_idx:end_idx].transpose(
                1, 2, 0, 3).reshape(img_h, img_w, -1)
    ...
    def store_frame(self, frame):
        ...
        idx: int
            ...
            To be used for `store_effect` later.
        """
        if self.obs is None:
