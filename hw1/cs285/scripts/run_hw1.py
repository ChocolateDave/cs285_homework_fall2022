...
class BC_Trainer(object):
    ...
    def run_training_loop(self):
        ...
        self.rl_trainer.run_training_loop(
            ...
            # Compared to classic behavior cloning,
            # DAgger relabels data with expert experience
...
def main():
    ...
    parser.add_argument('--expert_policy_file', '-epf',
                        type=str, required=True)
    # relative to where you're running this script from
    parser.add_argument('--expert_data', '-ed',
                        type=str, required=True)
    parser.add_argument('--env_name', '-env',
                        type=str, required=True,
                        help=f'choices: {", ".join(MJ_ENV_NAMES)}')
    parser.add_argument('--exp_name', '-exp',
                        type=str, required=True,
    ...
    parser.add_argument('--max_replay_buffer_size',
    ...
    if args.do_dagger:
        ...
        logdir_prefix = 'q1_'
        assert args.n_iter == 1, (
            'Vanilla behavior cloning collects expert data'
            'just once (n_iter=1)'
        )
