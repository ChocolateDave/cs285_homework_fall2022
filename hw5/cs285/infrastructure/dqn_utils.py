...
class PiecewiseSchedule(Schedule):
    def __init__(self,
                 ...
                 interpolation: Callable = linear_interpolation,
                 outside_value: bool = None) -> None:
        """Piecewise schedule.
        endpoints: [(int, int)]
            ...
            `time_a <= t < time_b` then value outputs `interpolation(value_a,
            value_b, alpha)` where alpha is a fraction of time passed between
            `time_a` and `time_b` for time `t`.
        ...
        outside_value: float
            if the value is requested outside of all the intervals sepecified
            in `endpoints` this value is returned. If None then AssertionError
            is raised when outside value is requested.
        """
        super().__init__()
...
#             # This can happen if the variables all depend on each other,
#             # or more likely if there's
#             # another variable outside of the list, that still needs
#             # to be initialized. This could be detected here,
#             # but life's finite.
#             raise Exception(
#                 "Cycle in variable dependencies, "
#                 "or extenrnal precondition unsatisfied.")
#         else:
...
class MemoryOptimizedReplayBuffer(object):
    def __init__(self, size, frame_history_len, lander=False, float_obs=False):
        ...
        For the tipical use case in Atari Deep RL buffer with 1M frames
        the total memory footprint of this buffer is 10^6 * 84 * 84
        bytes ~= 7 gigabytes
    ...
    def can_sample(self, batch_size):
        """Returns true if `batch_size` different transitions can be
    ...
    def _encode_observation(self, idx):
        ...
        if start_idx < 0 or missing_context > 0:
            ...
            return self.obs[start_idx:end_idx].transpose(1, 2, 0, 3) \
    ...
    def store_frame(self, frame: np.ndarray) -> int:
        ...
        -------
        idx: int
            Index at which the frame is stored. To be used for
            `store_effect` later.
        """
        if self.obs is None:
            self.obs = np.empty([self.size] + list(frame.shape),
                                dtype=np.float32 if self.float_obs
                                else np.uint8)
