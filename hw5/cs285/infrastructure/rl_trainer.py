...
class RL_Trainer(object):
    ...
    def __init__(self, params):
        ...
        if self.params['video_log_freq'] > 0:
            self.episode_trigger = lambda episode: \
                episode % self.params['video_log_freq'] == 0
        else:
        ...
        if 'non_atari_colab_env' in self.params and \
        ...
        ob_dim = self.env.observation_space.shape if img \
            else self.env.observation_space.shape[0]
        ac_dim = self.env.action_space.n if discrete \
    ...
    def run_training_loop(self,
                          ...
                          initial_expertdata: Optional[Any] = None,
        ...
        :param start_relabel_with_expert: iteration at which to
        start relabel with expert
        :param expert_policy:
        """
    ...
    def perform_logging(self,
                        ...
                        eval_policy: BasePolicy,
                        train_video_paths: str,
                        all_logs: List[Dict[str, Any]]) -> None:
    ...
    def dump_density_graphs(self, itr):
        ...
        def filepath(
            name): return self.params['logdir']+'/curr_{}.png'.format(name)
        ...
        H, xedges, yedges = np.histogram2d(states[:, 0], states[:, 1], range=[
                                           [0., 1.], [0., 1.]], density=True)
        ...
        plt.clf()
        exploration_values = self.agent.exploration_critic.qa_values(
            obs).mean(-1)
